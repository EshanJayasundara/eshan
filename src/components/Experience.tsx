...
function ExperienceCard({
  ...
  return (
    <motion.div
      ...
      className="glass p-6 rounded-2xl transition-all duration-150"
    >
      <div className="flex gap-4 md:gap-6 mb-3">
        ...
        <div className="flex-1 flex flex-col md:flex-row justify-between">
          <div>
            <h3 className="text-lg md:text-xl font-bold text-slate-800 dark:text-white mb-0.5">
              {role}
            </h3>
            <a
              href={website}
              ...
              className="text-primary font-semibold hover:underline flex items-center gap-1 text-sm md:text-base"
            >
              {company} <ExternalLink className="w-4 h-4" />
            </a>
            <span className="text-slate-500 dark:text-slate-400 font-medium text-xs md:hidden block mt-1">
              {period}
            </span>
          </div>
          <span className="text-slate-500 dark:text-slate-400 font-medium text-sm mt-2 md:mt-0 hidden md:block">
            {period}
          </span>
        </div>
      </div>
      <ul className="space-y-1.5">
        {highlights.map((highlight: string, idx: number) => (
          <li
            key={idx}
            className="text-slate-700 dark:text-slate-400 text-sm leading-relaxed flex items-start gap-2 ml-2"
          >
